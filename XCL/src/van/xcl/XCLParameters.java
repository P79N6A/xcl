...
public class XCLParameters {
	...
	public XCLParameters addKeyValue(String str, XCLContext context) {
		if (isKeyValue(str)) {
			...
			if (arr != null) {
				map.put(arr[0].replaceFirst(Constants.PARAS_PREFIX, "").trim(), context.resolveVar(arr[1].trim()));
			}
		}
		return this;
	}
	...
	public String toString() {
		...
		for (Entry<String, String> entry : map.entrySet()) {
			if (sb.length() > 0) {
				sb.append(Constants.PARAS_DELIMETER);
			}
			sb.append(Constants.PARAS_PREFIX);
			sb.append(entry.getKey());
			sb.append(Constants.PARAS_SPLITER);
	...
	public static XCLParameters resolveXCLParas(String string) {
		if (isXCLParas(string)) {
			...
			String[] arr = string.split(Constants.PARAS_DELIMETER);
			for (String str : arr) {
				String[] kv = splitKeyValue(str);
				if (kv != null) {
					mapping.map.put(kv[0].replaceFirst(Constants.PARAS_PREFIX, "").trim(), kv[1].trim());
				}
			}
			return mapping;
		}
	...
	public static String[] splitKeyValue(String str) {
		int idx = str.indexOf(Constants.PARAS_SPLITER);
		if (idx > 0)  {
			String[] arr = new String[2];
			arr[0] = str.substring(0, idx);
			arr[1] = str.substring(idx + Constants.PARAS_SPLITER.length());
			return arr;
		}
		return null;
	}
	...
	public static boolean isXCLParas(String string) {
		return string != null ? string.startsWith(Constants.PARAS_PREFIX) : false;
	}
	
	public static boolean isKeyValue(String string) {
		return string != null ? (string.startsWith(Constants.PARAS_PREFIX) && string.contains(Constants.PARAS_SPLITER)) : false;
	}
